...
-- ============================================================
-- Changes from v2.0:
--   - Renamed fin_cost_center to fin_revenue_center
--   - Renamed fin_journal_entry_line to fin_journal_entry_manager
--   - Renamed fin_voucher_line to fin_voucher_item_manager
--   - Added entry_type ENUM('ACTUAL','BUDGET') to fin_journal_entry
--   - Dropped fin_budget and fin_budget_line (simplified budget model)
--   - Added fin_voucher_item table
--   - Created credit_or_debit table
--   - Dropped old chart_of_account table
-- ============================================================
...
INSERT INTO credit_or_debit (type, description) VALUES
('DEBIT', 'Debit Entry'),
('CREDIT', 'Credit Entry')
...
-- Revenue Center (formerly Cost Center)
CREATE TABLE IF NOT EXISTS fin_revenue_center (
    id INT PRIMARY KEY AUTO_INCREMENT,
    revenue_center_code VARCHAR(20) NOT NULL UNIQUE,
    revenue_center_name VARCHAR(150) NOT NULL,
    parent_id INT NULL,
    organization_id INT,
    description VARCHAR(500),
    is_active TINYINT(1) DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (parent_id) REFERENCES fin_revenue_center(id),
    FOREIGN KEY (organization_id) REFERENCES general_organization_profile(id),
    INDEX idx_parent (parent_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Financial Partner (Customers/Vendors)
...
-- Journal Entry Header (supports ACTUAL and BUDGET entries)
CREATE TABLE IF NOT EXISTS fin_journal_entry (
    id INT PRIMARY KEY AUTO_INCREMENT,
    entry_number VARCHAR(30) NOT NULL UNIQUE,
    entry_type ENUM('ACTUAL', 'BUDGET') NOT NULL DEFAULT 'ACTUAL',
...
-- Journal Entry Manager (Line Items)
CREATE TABLE IF NOT EXISTS fin_journal_entry_manager (
    id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (revenue_center_id) REFERENCES fin_revenue_center(id),
...
-- Voucher Item (Base Item Definitions)
...
-- Voucher Item Manager (Association: Voucher <-> VoucherItem)
CREATE TABLE IF NOT EXISTS fin_voucher_item_manager (
    id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    FOREIGN KEY (voucher_item_id) REFERENCES fin_voucher_item(id),
    FOREIGN KEY (account_id) REFERENCES fin_chart_of_account(id),
    FOREIGN KEY (revenue_center_id) REFERENCES fin_revenue_center(id),
    FOREIGN KEY (credit_or_debit_id) REFERENCES credit_or_debit(id),
...
-- Root Chart of Accounts (Level 1)
...
-- Fiscal Periods for FY2026
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 1, 'January 2026', '2026-01-01', '2026-01-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 2, 'February 2026', '2026-02-01', '2026-02-28' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 3, 'March 2026', '2026-03-01', '2026-03-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 4, 'April 2026', '2026-04-01', '2026-04-30' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 5, 'May 2026', '2026-05-01', '2026-05-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 6, 'June 2026', '2026-06-01', '2026-06-30' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 7, 'July 2026', '2026-07-01', '2026-07-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 8, 'August 2026', '2026-08-01', '2026-08-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 9, 'September 2026', '2026-09-01', '2026-09-30' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 10, 'October 2026', '2026-10-01', '2026-10-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 11, 'November 2026', '2026-11-01', '2026-11-30' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);
INSERT INTO fin_fiscal_period (fiscal_year_id, period_number, period_name, start_date, end_date)
SELECT id, 12, 'December 2026', '2026-12-01', '2026-12-31' FROM fin_fiscal_year WHERE year_code = 'FY2026' ON DUPLICATE KEY UPDATE period_name = VALUES(period_name);

-- ============================================================
-- SCHEMA VERSION TRACKING
-- ============================================================
SELECT 'Finance Schema v2.1 installed successfully' AS status;
